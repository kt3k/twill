/**
 * The global property order (SPEC §11.4). Verbatim from the specification.
 *
 * @module
 */
...
const INDEX = new Map<string, number>(PROPERTY_ORDER.map((p, i) => [p, i]));

/** The index of a property in the global order, or -1. */
export function propertyIndex(property: string): number {
  return INDEX.get(property) ?? -1;
}
